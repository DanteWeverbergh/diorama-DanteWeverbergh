...
const Diorama = (props) => {
  ...
  useEffect(() => {
    if (found !== false) {
      actions.draaien.play();
    }
  });

  if (found !== false) {
    // if present is found
    // remove hidden class to display text
    // show only the present
    return (
    ...
    return (
      <group ref={group} {...props} dispose={null}>
        ...
        <group position={[17.69, 1.66, 11.97]} scale={[0.36, 0.36, 0.36]}>
          <mesh
            castShadow
            receiveShadow
            geometry={nodes.Cadeau.geometry}
